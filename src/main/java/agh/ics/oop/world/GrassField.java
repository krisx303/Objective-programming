...
public class GrassField extends AbstractWorldMap {
    ...
    private void generateGrass() {
        ...
        mapElements.add(new Grass(vector2d));
    }

    @Override
    public void updateVectors() {
        int[] values = {2147483647, 0, 2147483647, 0};
        mapElements.forEach(iMapElement -> {
            values[0] = Math.min(values[0], iMapElement.getPosition().x);
            values[2] = Math.min(values[2], iMapElement.getPosition().y);
            values[1] = Math.max(values[1], iMapElement.getPosition().x);
            values[3] = Math.max(values[3], iMapElement.getPosition().y);
        });
